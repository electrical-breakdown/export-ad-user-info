...
try {
    ...
    # Loop through $columnLabels and add the label to the first cell in each of the columns    
...
try {
    ...
    Write-Verbose "Writing user data to worksheet..."  -Verbose   
    ...
    foreach($adUser in $adUsers){      
        ...
        for($col = 1; $col -le $columnLabels.Count; $col ++){
            ...
            # On the last pass through the loop, select each column and resize it to fit the contents

            if($adUser -eq $adUsers[-1]){            

                $writeSheet.Cells(1, $col).EntireColumn.AutoFit() | Out-Null 
            }
        }                                                                                                                                                                       
